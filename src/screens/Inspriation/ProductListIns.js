...
export default class ProductListIns extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dataSource: {},
  ...
  var param = {
       ...
       Name: this.props.route.params.catID,
       WebCountryCode: 'KW',
       UserID: globalVariables.userId === '' ? 0 : globalVariables.userId,
       Corpcentreby: '2',
       CultureId: globalVariables.CultureId,
    };
    if (this.props.route.params.type === 'onlineShop')
    {
      param ={
       Category: this.props.route.params.catID,
       SubCategory: this.props.route.params.subCatID,
       Id: this.props.route.params.itemID,
       Name: this.props.route.params.catID,
       WebCountryCode: 'KW',
       UserID: globalVariables.userId === '' ? 0 : globalVariables.userId,
    ...
    if (this.props.route.params.type === 'inspire')
    {
      param ={
        Id: this.props.route.params.catID,
        Name: this.props.route.params.subCatID,
       WebCountryCode: 'KW',
       UserID: globalVariables.userId === '' ? 0 : globalVariables.userId,
       Corpcentreby: '2',
       CultureId: globalVariables.CultureId,
  ...
  ApicallFilter(arrBrand,arrCat,arrSubCat,priceMin,priceMax,sortedBy)
  {
    ...
    if (this.props.route.params.type === 'onlineShop')
    {
      param ={
        ...
        Corpcentreby: '2',
        CultureId: globalVariables.CultureId,
        Arry_Category: this.state.arryCategory,
        Arry_SubCategory:this.state.arrySubCategory,
        Arry_Brand: this.state.arryBrand,
   ...
   new ApiHelper().serviceCallGet(
      param,
      this.props.route ? this.props.route.params.filteredProduct : `${res.api.baseUrlCommon}${res.api.Get_NewCollectionAllProducts_Filterwise}`,
      (data, err) => {
        if (err) {
          alert(err);
        } else {
    ...
    return (
      ...
      <View style={{width:'45%', flexDirection: 'column',margin:8,backgroundColor:res.color.whilteColor,borderRadius:10,marginLeft:10,shadowOpacity:5.0,shadowColor:res.color.lightGrayColor}}>
        <TouchableOpacity onPress={() => this.onPressCell(item)}>
         ...
         <View  style={{height:40,justifyContent:'space-between',flexDirection:'row', alignItems:'center',marginLeft:10,marginRight:10,marginTop:8}}>
         ...
         {this.renderTopView(item)}
            ...
            <TouchableOpacity style={{width:30,height:30}} onPress={()=>{globalVariables.userId != 0 ? item.IsWishListItem === false ? this.addToWishList(item) : this.deleteToWishList(item) : Toast.show(res.strings.pleaseLogin)}}>
              <Image source={res.ImageAssets.favUnselectIcon} />
            </TouchableOpacity>
          </View>
        <View style={{alignItems:'center'}}>
        <Image style={{width:'80%',height: 150,resizeMode:'contain'}} source={{
            uri: res.api.baseUrlProductImage.concat(
              item.Image
              ),
          }} />
        <Text style={{marginLeft:8,marginRight:8}}>{item.XName} </Text>  
        ...
        {item.OldPrice > 0.0 ? <Text style={{textDecorationStyle: 'solid',textDecorationLine: 'line-through',color:res.color.midGrayColor,fontFamily:res.font.ragularFont}}>{item.OldPrice.toFixed(3) + ' ' + item.Currency} </Text> : null}
